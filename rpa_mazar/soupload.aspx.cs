...
namespace rpa_mazar
{
    public partial class soupload : System.Web.UI.Page
    {
                    ...
                    if (ReadExcelToDataTable())
                    {
                        if (validateInputExcelColumns())
                        {
                            ...
                            if (dtInputExcel.Rows.Count > 0)
                            {
                                ...
                                dtTemp = objSQL.DataTable_return("select max(ReqID) as lastReqId from salesorderinput;");
                                ...
                                dtTemp = objSQL.DataTable_return("select max(CaseCustomerID) as lastCaseCustomerId from salesorderinput;");
                                ...
                                foreach (DataRow dr in dtInputExcel.Rows)
                                {
                                    try
                                    {
                                        ...
                                        objSQL.ExecuteNonQuery_IUD("insert into salesorderinput (CaseCustomerID,CustomerCode,SalesOrderNo,SPOCEmail,ReqID,EmailStatus,Status,CreatedDate,Comments) values (" +
        ...
        protected void lnk_FormatDownload_ServerClick(object sender, EventArgs e)
        {
            ...
            Response.AppendHeader("Content-Disposition", "attachment; filename=soformat.xlsx");
            Response.TransmitFile(ConfigurationManager.AppSettings["basefilepathfordownloads"] + "soformat.xlsx");
            Response.End();
        }
