...
namespace rpa_mazar
{
    public partial class FreightMis : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                pendingrequests.InnerHtml = objSQL.DataTable_return("select count(*) from freightuserinput where SPOCEmail='" + Session["username"].ToString() + "' and Status=2;").Rows[0][0].ToString();
                ...
                approvedrequets.InnerHtml = objSQL.DataTable_return("select count(*) from freightuserinput where SPOCEmail='" + Session["username"].ToString() + "' and Status=3;").Rows[0][0].ToString();
                rejectedrequests.InnerHtml = objSQL.DataTable_return("select count(*) from freightuserinput where SPOCEmail='" + Session["username"].ToString() + "' and Status=4;").Rows[0][0].ToString();
            }
        }
        ...
        protected void setRequestIds()
        {
            try
            {
                ...
                if (Session["username"].ToString() == "rpa-admin@dalmiabharat.com")
                {
                    ...
                    dtTemp = objSQL.DataTable_return("select distinct(ReqID) from freightuserinput order by CreatedDate desc limit 100;");
                ...
                {
                    requestids.Items.Clear();
                    dtTemp = objSQL.DataTable_return("select distinct(ReqID) from freightuserinput where SPOCEmail='" + Session["username"].ToString() + "' order by CreatedDate desc limit 20;");
                    requestids.DataSource = dtTemp;
                    requestids.DataTextField = "ReqID";
                    requestids.DataValueField = "ReqID";
                    requestids.DataBind();
        ...
        protected void requestids_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //dtTemp = objSQL.DataTable_return("SELECT distinct t1.`REQID`,t1.`RPAID`,t2.approvalemail 'APPROVAL EMAIL', t1.SPOCEMAIL AS 'SPOC EMAIL', case when t1.status = 0 then 'New Request' when t1.status = 2 then 'Pending for Approval' when t1.status = 3 then 'Approved' when t1.status = 4 then 'Freight Created' when t1.Status = 5 then 'Freight Updation' when t1.status = 6 then 'Rejected/Validation Failed'  when t1.status = 7 then 'Freight Processed Successfully' end as `CURRENT STATUS`, t1.Description, t1.createdDate 'CREATED DATE', t1.processeddate 'PROCESSED DATE', CASE WHEN t1.`FreightType` = 'P' THEN 'Primary' ELSE 'Secondary' END AS 'TYPE', CASE WHEN t1.`FreightTypeRoute` = 'N' THEN 'New Route' ELSE 'Secondary Route' END AS 'ROUTE', t1.`ZONE` `TRANSPORTATION PLANNING POINT`, t1.`DISTRIBUTIONCHANNEL`, t1.`SHIPMENTROUTE`, t1.`SHIPPINGTYPEFORSHIPMENTSTAGE`, t1.`PACKAGINGMATERIALS`, t1.FromDate 'FROM DATE', t1.ToDate 'TO DATE', t1.Rate 'RATE', t1.`COMMENTS` FROM `FreightUserInput` t1 left join `freightapprovaltransaction` t2 on t1.RPAID = t2.RPAID where t2.ApprovalStatus=1 and t2.level=1 and t1.reqid = " + requestids.SelectedValue + "; ");
            dtTemp = objSQL.DataTable_return("drop table if exists t2; " +
            "create temporary table t2 select distinct RPAID, ApprovalEmail, ApprovalStatus, Level from freightapprovaltransaction " +
            "where RPAID in (select RPAID from freightuserinput where ReqID = " + requestids.SelectedValue + " ) and approvalstatus = 1 and level = 1; " +
            "SELECT distinct t1.`REQID`,t1.`RPAID`,t1.SPOCEMAIL AS 'SPOC EMAIL', " +
            "case when t1.status = 0 then 'New Request' when t1.status = 2 then 'Pending for Approval' when t1.status = 3 then 'Approved' when t1.status = 4 then 'Freight Created'" +
            "when t1.Status = 5 then 'Freight Updation' when t1.status = 6 then 'Rejected/Validation Failed' " +
            "when t1.status = 7 then 'Freight Processed Successfully' end as `CURRENT STATUS`, t1.Description, t1.createdDate 'CREATED DATE'," +
            "t1.processeddate 'PROCESSED DATE', CASE WHEN t1.`FreightType` = 'P' THEN 'Primary' ELSE 'Secondary' END AS 'TYPE', " +
            "CASE WHEN t1.`FreightTypeRoute` = 'N' THEN 'New Route' ELSE 'Secondary Route' END AS 'ROUTE', t1.`ZONE` `TRANSPORTATION PLANNING POINT`," +
            "t1.`DISTRIBUTIONCHANNEL`, t1.`SHIPMENTROUTE`, t1.`SHIPPINGTYPEFORSHIPMENTSTAGE`, t1.`PACKAGINGMATERIALS`, t1.FromDate 'FROM DATE'," +
            "t1.ToDate 'TO DATE', t1.Rate 'RATE', t1.`COMMENTS`," +
            "CASE WHEN t1.status = 2 and t2.ApprovalStatus=1 and t2.level=1 THEN t2.ApprovalEmail ELSE '' END AS `Current Approver`" +
            "FROM `FreightUserInput` t1 LEFT JOIN t2 ON t1.RPAID = t2.RPAID left join `freightapprovaltransaction` t3 on t1.RPAID = t3.RPAID where t1.reqid = " + requestids.SelectedValue + "; ");
            gv_requeststatus.DataSource = dtTemp;
            gv_requeststatus.DataBind();
        }
    }
