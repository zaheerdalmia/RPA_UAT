...
namespace rpa_mazar
{
    public partial class contractlongtermmaster : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!(Session["username"] == null))
            {
                ...
                {
                    Logger.Log("Line No 34 ContractLongTemp-Master", w);
                }
                try
                {
            ...
            else
            {
                using (StreamWriter w = new StreamWriter(ConfigurationManager.AppSettings["errlog"].ToString() + DateTime.Now.ToString("dd_MMM_yyyy") + ".txt", true))
                {
                    Logger.Log("LN 112 ContractLohgTerm-Master \n", w);
                }
        ...
        {
            string ContractFlag = "LT";
            dtTemp = objSQL.DataTable_return("SELECT distinct cc_appro.ApprovalEmail,ncc.ReqID,ncc.ContractID,cc_appro.ApprovalID,ncc.zone,ncc.SLNo,ncc.CustGrpCode,ncc.CustGrpDescrip,ncc.LT_ISPrice,ncc.ValidityFrom,ncc.ValidityTo,ncc.ContractRemarks FROM ntcontractupd_input ncc inner join ntcontractupd_approvaltransaction cc_appro on cc_appro.ContractID = ncc.ContractID and cc_appro.ReqID = ncc.ReqID and ncc.ContractFlag = '" + ContractFlag + "' where cc_appro.Level = 1 and cc_appro.ApprovalID = '" + approverCode + "' and ncc.Status in (2,4);");
            
            if(dtTemp.Rows.Count < 1)
            {
                dvCLTMaster.Visible = false;
            }
