...
namespace rpa_mazar
{
    public partial class contractshortcloser : System.Web.UI.UserControl
    {
        ...
        {
            dtTemp = objSQL.DataTable_return("SELECT distinct cc_appro.ApprovalEmail,ncc.ReqID,ncc.ContractID,cc_appro.ApprovalID,ncc.zone,ncc.ContractNo,ncc.CustCode,ncc.CustName,ncc.BalContractQty,ncc.ContractPrice,ncc.ReasonOfRejection,ncc.ContractRemarks FROM ntcontractupd_input ncc inner join ntcontractupd_approvaltransaction cc_appro on cc_appro.ContractID = ncc.ContractID and cc_appro.ReqID = ncc.ReqID and ncc.ContractFlag = '" + ContractFlag + "' where cc_appro.Level = 1 and cc_appro.ApprovalID = '" + approverCode + "' and ncc.Status in(2,4);");
         
            if(dtTemp.Rows.Count > 0)
            {
                dvShortCloser.Visible = true;
            }
