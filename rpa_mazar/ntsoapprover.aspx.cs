...
namespace rpa_mazar
{
    public partial class ntsoapprover : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                /*dtTemp = objSQL.DataTable_return("select Approval_ID from salesorderapprovalmaster where ApprovalEmail='" + Session["username"].ToString() + "';");
                if (dtTemp.Rows.Count > 0)
        ...
        protected void GetDataForApproval()
        {
            ...
            "(Select * From nontrade_salesorderprocessing where caseCustomerID in (select caseCustomerID from nontrade_salesorderapprovaltransaction where Approval_ID = " + approverCode + " " +
            "and Approval_status = 1 and level = 1 and caseCustomerID in (select caseCustomerID from nontrade_salesorderinput where status = 2))) t, " +
            "nontrade_salesorderinput si where si.CaseCustomerID = t.CaseCustomerID;");*/

            dtTemp = objSQL.DataTable_return("Select distinct SPOCEmail, Approver, si.CaseCustomerID, Zone, State, Case when Division = 1 then 'Cement' when Division = 2 then 'Clinker' when Division = 3 then 'Fly Ash' when Division = 4 then 'Power' when Division = 5 then 'Scrap' when Division = 6 then 'NewBuildingSolutions' when Division = 7 then 'Limestone Chips' when Division = 8 then 'Ready Mix Cement' when Division = 9 then 'Fly Ash Bricks' when Division = 10 then 'Slag' when Division = 11 then 'Asset' when Division = 12 then 'Others' when Division = 13 then 'Coal' when Division = 14 then 'Bricks' when Division = 15 then 'Craft Beton' end as Division,CustGrp2, Case when PayTerms = 'ZS01' then 'PAYABLE IN 1 DAY' when PayTerms = 'ZS02' then 'PAYABLE IN 3 DAYS' when PayTerms = 'ZS03' then 'PAYABLE IN 7 DAYS' when PayTerms = 'ZS04' then 'PAYABLE IN 15 DAYS' when PayTerms = 'ZS05' then 'PAYABLE IN 30 DAYS' when PayTerms = 'ZS06' then 'PAYABLE IN 40 DAYS' when PayTerms = 'ZS07' then 'PAYABLE IN 10 DAYS' when PayTerms = 'ZS08' then 'PAYABLE IN 35 DAYS' when PayTerms = 'ZS19' then 'PAYABLE IN 21 DAYS' when PayTerms = 'ZS20' then 'PAYABLE IN 60 DAYS' when PayTerms = 'ZS21' then 'PAYABLE IN 45 DAYS' when PayTerms = 'ZS25' then 'PAYABLE IN 75 DAYS' when PayTerms = 'ZS26' then 'PAYABLE IN 18 DAYS' when PayTerms = 'ZS27' then 'PAYABLE IN 80 DAYS' when PayTerms = 'ZS28' then 'PAYABLE IN 100 DAYS' when PayTerms = 'ZS29' then 'PAYABLE IN 120 DAYS' when PayTerms = 'Z001' then 'Advance' end as PayTerms,CustomerGroup1Description, Case when DistributionChannel = 1 then 'Trade' when DistributionChannel = 2 then 'Institutional' when DistributionChannel = 3 then 'Stock Transfer' when DistributionChannel = 4 then 'Self Consumption' when DistributionChannel = 5 then 'Inter Company' when DistributionChannel = 6 then 'Export Sales' end as DistributionChannel, Division, si.CustomerCode, Name,  TotalCreditLimit, PremanentCreditLimit, TempCreditLimit, ProposedCreditLimit, 'OpenCredit/Security', SecurityDeposit, SSD, BankGuarantee, ChequeUnderClearance, TotalOutstanding, TotalNotDue, Days0To30, Days30To45, Days45To60, Days60To75, Days75To90, Days90To105, Days105To120, ABOVE120DAYS, SOValue, DeliveryValue, TotalOsSoDelivery, STEP_1, STEP_2, NODBlock, NetCreditAmount, FinalOutstanding from (Select * From nontrade_salesorderprocessing where caseCustomerID in (select caseCustomerID from nontrade_salesorderapprovaltransaction where ApprovalEmail = '" + Convert.ToString(Session["username"]) + "' and Approval_status = 1 and level = 1 and caseCustomerID in (select caseCustomerID from nontrade_salesorderinput where status = 2))) t, nontrade_salesorderinput si where si.CaseCustomerID = t.CaseCustomerID;");
        ...
        protected void ApproveRequests()
        {
            try
            {
                ...
                objSQL.ExecuteNonQuery_IUD("update nontrade_salesorderapprovaltransaction set Approval_Status=2,ApproverResponseDate='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where level=1 and Approval_Status=1 and ApprovalEmail='" + Convert.ToString(Session["username"]) + "' and CaseCustomerID in (" + strApproveList + "); ");
                
                foreach (string strCCI in strApproveList.Split(','))
                {
                    dtTemp = objSQL.DataTable_return("select * from nontrade_salesorderapprovaltransaction where level=0 and Approval_Status=0 and CaseCustomerID=" + strCCI + " order by Priority limit 1;");
                    dtTemp1 = objSQL.DataTable_return("select * from nontrade_salesorderapprovaltransaction where level=1 and Approval_Status=1 and CaseCustomerID=" + strCCI + " order by Priority limit 1;");
                    ...
                    {
                        objSQL.ExecuteNonQuery_IUD("update nontrade_salesorderapprovaltransaction set level = 1 where TransactionID =" + dtTemp.Rows[0]["TransactionID"] + ";");
                    }
                    else
                    {
                        objSQL.ExecuteNonQuery_IUD("update nontrade_salesorderinput set Status=3,`ApproverResponseDate`='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where CaseCustomerID= " + strCCI + " and status=2;");
                    }
                }
                //GetDataForApproval();
                //lbl_Message.Attributes.Add("style", "color:green; font-weight:bold;");
        ...
        protected void RejectRequests()
        {
            try
            {
                objSQL.ExecuteNonQuery_IUD("update nontrade_salesorderapprovaltransaction set Approval_Status=3,ApproverResponseDate='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where level=1 and Approval_Status=1 and ApprovalEmail='" + Convert.ToString(Session["username"]) + "' and CaseCustomerID in (" + strRejectList + "); ");
                foreach (string strCCI in strRejectList.Split(','))
                {
                    objSQL.ExecuteNonQuery_IUD("update nontrade_salesorderinput set Status=4,ApproverResponseDate='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "', Remarks='Fail',Description='Rejection from Approver', ProcessedDate='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "'  where CaseCustomerID= " + strCCI + " and Status=2;");
                }
                //GetDataForApproval();
                //lbl_Message.Attributes.Add("style", "color:green; font-weight:bold;");
                //lbl_Message.InnerText = "Yaay! Transaction Approve / Reject Updated Successfully";
        ...
        protected void setRequestIds()
        {
            try
            {
                ...
                dtTemp = objSQL.DataTable_return("select distinct(ReqID) from nontrade_salesorderinput where CaseCustomerID in (select CaseCustomerID from nontrade_salesorderapprovaltransaction where ApprovalEmail='" + Convert.ToString(Session["username"]) + "' and Approval_Status=1) and Status=2;");
        ...
        protected void requestids_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                dtTemp = objSQL.DataTable_return("Select distinct SPOCEmail, Approver, si.CaseCustomerID, Zone, State, Case when Division = 1 then 'Cement' when Division = 2 then 'Clinker' when Division = 3 then 'Fly Ash' when Division = 4 then 'Power' when Division = 5 then 'Scrap' when Division = 6 then 'NewBuildingSolutions' when Division = 7 then 'Limestone Chips' when Division = 8 then 'Ready Mix Cement' when Division = 9 then 'Fly Ash Bricks' when Division = 10 then 'Slag' when Division = 11 then 'Asset' when Division = 12 then 'Others' when Division = 13 then 'Coal' when Division = 14 then 'Bricks' when Division = 15 then 'Craft Beton' end as Division,CustGrp2, Case when PayTerms = 'ZS01' then 'PAYABLE IN 1 DAY' when PayTerms = 'ZS02' then 'PAYABLE IN 3 DAYS' when PayTerms = 'ZS03' then 'PAYABLE IN 7 DAYS' when PayTerms = 'ZS04' then 'PAYABLE IN 15 DAYS' when PayTerms = 'ZS05' then 'PAYABLE IN 30 DAYS' when PayTerms = 'ZS06' then 'PAYABLE IN 40 DAYS' when PayTerms = 'ZS07' then 'PAYABLE IN 10 DAYS' when PayTerms = 'ZS08' then 'PAYABLE IN 35 DAYS' when PayTerms = 'ZS19' then 'PAYABLE IN 21 DAYS' when PayTerms = 'ZS20' then 'PAYABLE IN 60 DAYS' when PayTerms = 'ZS21' then 'PAYABLE IN 45 DAYS' when PayTerms = 'ZS25' then 'PAYABLE IN 75 DAYS' when PayTerms = 'ZS26' then 'PAYABLE IN 18 DAYS' when PayTerms = 'ZS27' then 'PAYABLE IN 80 DAYS' when PayTerms = 'ZS28' then 'PAYABLE IN 100 DAYS' when PayTerms = 'ZS29' then 'PAYABLE IN 120 DAYS' when PayTerms = 'Z001' then 'Advance' end as PayTerms,CustomerGroup1Description, Case when DistributionChannel = 1 then 'Trade' when DistributionChannel = 2 then 'Institutional' when DistributionChannel = 3 then 'Stock Transfer' when DistributionChannel = 4 then 'Self Consumption' when DistributionChannel = 5 then 'Inter Company' when DistributionChannel = 6 then 'Export Sales' end as DistributionChannel, Division, si.CustomerCode, Name,  TotalCreditLimit, PremanentCreditLimit, TempCreditLimit, ProposedCreditLimit, 'OpenCredit/Security', SecurityDeposit, SSD, BankGuarantee, ChequeUnderClearance, TotalOutstanding, TotalNotDue, Days0To30, Days30To45, Days45To60, Days60To75, Days75To90, Days90To105, Days105To120, ABOVE120DAYS, SOValue, DeliveryValue, TotalOsSoDelivery, STEP_1, STEP_2, NODBlock, NetCreditAmount, FinalOutstanding from (Select * From nontrade_salesorderprocessing where caseCustomerID in (select caseCustomerID from nontrade_salesorderapprovaltransaction where ApprovalEmail = '" + Convert.ToString(Session["username"]) + "' and Approval_status = 1 and level = 1 and caseCustomerID in (select caseCustomerID from nontrade_salesorderinput where status = 2))) t, nontrade_salesorderinput si where si.CaseCustomerID = t.CaseCustomerID and si.ReqID = '" + requestids.SelectedValue + "';");
                gv_approvallist.DataSource = dtTemp;
                gv_approvallist.DataBind();
            }
        }
