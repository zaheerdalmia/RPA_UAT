...
namespace rpa_mazar
{
    public partial class contractmodificationmaterial : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!(Session["username"] == null))
            {
                ...
                {
                    Logger.Log("LNO 35- ContractModification \n", w);
                }
                try
                {
            ...
            else
            {
                using (StreamWriter w = new StreamWriter(ConfigurationManager.AppSettings["errlog"].ToString() + DateTime.Now.ToString("dd_MMM_yyyy") + ".txt", true))
                {
                    Logger.Log("LN-113 ContractModification \n", w);
                }
            ...
            if (!IsPostBack)
            {
                ...
                //dtTemp = objSQL.DataTable_return("select ApprovalID from ntcontractupd_approvalmaster where ApprovalEmail='" + Session["username"].ToString() + "';");
                if (dtTemp.Rows.Count > 0)
        ...
        protected void RejectRequests()
        {
            ...
            try
            {
                objSQL.ExecuteNonQuery_IUD("update ntcontractupd_approvaltransaction set Approval_Status=3, ApproverResponseDate=now() where level=1 and ContractFlag = '" + ContractFlag + "' and Approval_Status in (0,1) and Approval_Id=" + approverCode + " and ContractID in (" + strRejectList + "); ");
        ...
        {
            string ContractFlag = "M";

            
            dtTemp = objSQL.DataTable_return("SELECT distinct cc_appro.ApprovalEmail,ncc.ReqID,ncc.ContractID,cc_appro.ApprovalID,ncc.zone,ncc.ContractNo,ncc.ContractPrice,ncc.ModifiedMaterialCode,ncc.ExistingMaterialCode,ncc.ContractRemarks FROM ntcontractupd_input ncc inner join ntcontractupd_approvaltransaction cc_appro on cc_appro.ContractID = ncc.ContractID and cc_appro.ReqID = ncc.ReqID and ncc.ContractFlag = '" + ContractFlag + "' where cc_appro.Level = 1 and cc_appro.ApprovalID = '" + approverCode + "' and ncc.Status in (2,4);");
            // End
