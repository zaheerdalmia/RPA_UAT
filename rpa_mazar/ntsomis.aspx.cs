...
namespace rpa_mazar
{
    public partial class ntsomis : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                pendingrequests.InnerHtml = objSQL.DataTable_return("select count(*) from nontrade_salesorderinput where SPOCEmail='" + Session["username"].ToString() + "' and Status=2;").Rows[0][0].ToString();
                ...
                approvedrequets.InnerHtml = objSQL.DataTable_return("select count(*) from nontrade_salesorderinput where SPOCEmail='" + Session["username"].ToString() + "' and Status=3;").Rows[0][0].ToString();
                rejectedrequests.InnerHtml = objSQL.DataTable_return("select count(*) from nontrade_salesorderinput where SPOCEmail='" + Session["username"].ToString() + "' and Status=4;").Rows[0][0].ToString();
            }
        }
        ...
        protected void btn_reportdownload_ServerClick(object sender, EventArgs e)
        {
            ...
            //    "(select distinct(caseCustomerID) from nontrade_salesorderinput where SPOCEmail = '" + Session["username"].ToString() + "') and " +
            ...
            //", t.Approval_Status, si.Remarks from salesorderapprovaltransaction t right join nontrade_salesorderinput si " +
        ...
        protected void setRequestIds()
        {
            try
            {
                ...
                if (Session["username"].ToString() == "rpa-admin@dalmiabharat.com")
                {
                    ...
                    dtTemp = objSQL.DataTable_return("select distinct(ReqID) from nontrade_salesorderinput order by CreatedDate desc limit 100;");
                ...
                {
                    requestids.Items.Clear();
                    dtTemp = objSQL.DataTable_return("select distinct(ReqID) from nontrade_salesorderinput where SPOCEmail='" + Session["username"].ToString() + "' order by CreatedDate desc limit 20;");
                    requestids.DataSource = dtTemp;
                    requestids.DataTextField = "ReqID";
                    requestids.DataValueField = "ReqID";
                    requestids.DataBind();
        ...
        protected void requestids_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            "SPOCEmail as SPOC, " +
            "(select count(*) from nontrade_salesorderinput where ReqID = " + requestids.SelectedValue + ") as 'Total Records', " +
            "(select count(*) from nontrade_salesorderinput where Description in ('Business Exception', 'SPOC invalid', 'Not to be Released') and ReqID =  " + requestids.SelectedValue + ") as 'Rejected in Validation', " +
            "(select count(*) from nontrade_salesorderinput where status = 2 and ReqID =  " + requestids.SelectedValue + ") as 'Pending for Approval', " +
            "(select count(*) from nontrade_salesorderinput where Description in ('Rejection from Approver') and ReqID =  " + requestids.SelectedValue + ") as 'Records Rejected by Approver', " +
            "(select count(*) from nontrade_salesorderinput where Description in ('RecordNotFoundInSAP', 'ID is not maintained in tcode ZSD_CRM') and Status = 4 and ReqID =  " + requestids.SelectedValue + ") as 'Records Unsuccessful in Processing', " +
            "(select count(*) from nontrade_salesorderinput where Status = 5 and ReqID =  " + requestids.SelectedValue + ") as 'Records Successful in Processing' from nontrade_salesorderinput where ReqID =  " + requestids.SelectedValue + ") t limit 1;");
            gv_top20requests.DataSource = dtTemp;
            gv_top20requests.DataBind();

            dtTemp = objSQL.DataTable_return("drop table if exists t1;  " +
           "create temporary table t1 select distinct CaseCustomerID, ApprovalEmail, Approval_Status, Level from salesorderapprovaltransaction where CaseCustomerID in (select CaseCustomerID from nontrade_salesorderinput " +
           ...
           "from nontrade_salesorderinput s1 left join t1 on s1.CaseCustomerID = t1.CaseCustomerID left join salesorderprocessing s2 on s1.CaseCustomerID = s2.CaseCustomerID where s1.ReqID = " + requestids.SelectedValue + "; ");
