...
namespace rpa_mazar
{
    public partial class contractmodificationplant : System.Web.UI.UserControl
    {
        ...
        protected void ApproveRequests()
        {
            ...
            try
            {
                ...
                objSQL.ExecuteNonQuery_IUD("update ntcontractupd_approvaltransaction set Approval_Status=2, ApproverResponseDate=now() where level=1 and ContractFlag = '" + ContractFlag + "' and Approval_Status in (0,1) and Approval_Id=" + approverCode + " and ContractID in (" + strApproveList + "); ");
                //END
        ...
        {
            string ContractFlag = "P";
            dtTemp = objSQL.DataTable_return("SELECT distinct cc_appro.ApprovalEmail,ncc.ReqID,ncc.ContractID,cc_appro.ApprovalID,ncc.zone,ncc.ContractNo,ncc.ContractPrice,ncc.ModifiedPlantCode,ncc.ExistingPlantCode,ncc.ContractRemarks FROM ntcontractupd_input ncc inner join ntcontractupd_approvaltransaction cc_appro on cc_appro.ContractID = ncc.ContractID and cc_appro.ReqID = ncc.ReqID and ncc.ContractFlag = '" + ContractFlag + "' where cc_appro.Level = 1 and cc_appro.ApprovalID = '" + approverCode + "' and ncc.Status in (2,4);");
                        
            if(dtTemp.Rows.Count > 0)
            {
                dvCMPlant.Visible = false;
            }
